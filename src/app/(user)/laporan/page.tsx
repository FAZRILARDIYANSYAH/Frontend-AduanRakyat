...
import { Search, RefreshCw, Inbox, SlidersHorizontal } from "lucide-react";
import { motion } from "framer-motion";
import { useLaporan } from "@/hooks/useLaporan";
import ReportCard from "@/components/user/ReportCard";
import ReportDetailPanel from "@/components/user/ReportDetailPanel";
...
export default function LaporanPage() {
  const { reports, loading, error, refetch } = useLaporan();
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {loading ? (
        <div className="grid md:grid-cols-2 gap-5">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white rounded-[2rem] p-6 animate-pulse h-52" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <motion.div
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {filtered.map((report) => (
            <ReportCard
              key={report.id}
              report={report}
              onClick={(r) => setSelectedReport(r)}  // ← buka panel
            />
          ))}
        </div>
      )}

      {/* Detail Panel — di luar grid supaya overlay bisa full-screen */}
      <ReportDetailPanel
